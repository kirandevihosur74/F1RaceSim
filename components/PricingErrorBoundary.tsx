...
class PricingErrorBoundary extends Component<Props, State> {
  constructor(props: Props) {
    ...
    this.state = { 
      hasError: false, 
      isPkMatchError: false 
    }
  }

  static getDerivedStateFromError(error: Error): State {
    const isPkMatchError = error.message.includes('pk.match') || 
  ...
  componentDidCatch(error: Error, errorInfo: ErrorInfo) {
    ...
    const isPkMatchError = error.message.includes('pk.match') || 
                           ...
                           error.message.includes('schema')
    if (isPkMatchError) {
      console.error('Database error detected in PricingErrorBoundary')
    }
  ...
  render() {
    if (this.state.hasError) {
      if (this.state.isPkMatchError) {
        // Special handling for pk.match errors
        return (
          <div className="min-h-screen bg-gray-50 dark:bg-gray-900 flex items-center justify-center p-4">
            <div className="max-w-md w-full bg-white dark:bg-gray-800 rounded-lg shadow-lg p-6 text-center">
              ...
              <p className="text-gray-600 dark:text-gray-400 mb-4">
                We're experiencing a temporary issue with our database. This is a known issue that we're working to resolve.
              </p>
              ...
              {process.env.NODE_ENV === 'development' && this.state.error && (
                <details className="mt-4 text-left">
                  ...
                  <div className="mt-2 p-3 bg-gray-100 dark:bg-gray-700 rounded text-xs font-mono text-gray-800 dark:text-gray-200 overflow-auto">
                    ...
                    <div><strong>Type:</strong> pk.match database error</div>
